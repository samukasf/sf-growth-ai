...
export default async function MetaConnectPage({ searchParams }: ConnectPageProps) {
  ...
  if (company) {
    try {
      const connection = await findMetaOAuthConnection(company.id);
      pageName = connection?.pageName ?? connection?.pageId ?? null;
    } catch {
      pageName = null;
    }
  }
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-xl flex-col justify-center gap-6 px-6 py-16">
      <div>
        ...
        <h1 className="mt-2 text-3xl font-semibold tracking-tight text-zinc-100">Meta</h1>
        <p className="mt-2 text-sm text-zinc-400">
          Conecte Facebook Page / Instagram para o Samuel ler métricas reais.
        </p>
      </div>

      {params.connected === "1" ? (
        <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-200">
          Conta Meta conectada com sucesso{pageName ? ` (${pageName})` : ""}.
        </div>
      ) : null}
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
        <dl className="space-y-3 text-sm">
          ...
          <div className="flex justify-between gap-4">
            ...
            <dd className="text-zinc-100">
              {pageName ? `Conectado · ${pageName}` : "Desconectado"}
            </dd>
          </div>
        </dl>
        ...
        <div className="mt-6 flex flex-col gap-3">
          <a
            href={authorizeHref}
            className="inline-flex items-center justify-center rounded-xl bg-[#1877F2] px-4 py-2.5 text-sm font-semibold text-white transition hover:brightness-110"
          >
            {pageName ? "Reconectar Meta" : "Conectar Meta"}
          </a>
          <Link
