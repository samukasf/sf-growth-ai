      ...
      <div className="grid min-h-0 lg:grid-cols-[250px_1fr]">
        <aside className="border-b border-white/[.06] bg-black/10 p-4 lg:border-b-0 lg:border-r lg:p-5">
            ...
            {(["today", "week"] as const).map((option) => (
              <button
                ...
                className={cn(
                  ...
                  view === option
                    ? "border-cyan-300/25 bg-cyan-300/[.09] text-cyan-50"
                    : "border-white/[.06] bg-white/[.025] text-white/45 hover:text-white/75",
                )}
              >
                {option === "today" ? "Hoje" : "Esta semana"}
              </button>
            ))}
          </div>
          ...
          <button
            type="button"
            onClick={() => void load()}
            className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl border border-white/[.07] px-3 py-2.5 text-xs text-white/50 transition hover:bg-white/[.04] hover:text-white/80"
          >
        ...
        <div className="min-h-0 p-4 sm:p-5 lg:p-6">
          <div className="mb-4 flex items-end justify-between gap-3">
            <div>
              <p className="text-[10px] uppercase tracking-[.18em] text-white/25">{view === "today" ? "Hoje" : "Próximos compromissos"}</p>
              <strong className="mt-1 block text-sm text-white/75">{filtered.length} evento(s)</strong>
            </div>
          </div>
          ...
          {loading ? (
            <div className="grid gap-3">
              {Array.from({ length: 4 }, (_, index) => (
                <div key={index} className="h-24 animate-pulse rounded-2xl border border-white/[.05] bg-white/[.025]" />
              ))}
            </div>
          ) : filtered.length === 0 ? (
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div className="min-w-0">
                      ...
                      <div className="mt-2 flex flex-wrap gap-x-4 gap-y-2 text-[11px] text-white/38">
                        <span className="flex items-center gap-1.5"><Clock3 className="size-3.5" />{event.allDay ? "Dia inteiro" : `${formatTime(event.start)} – ${formatTime(event.end)}`}</span>
                    ...
                    <button
                      type="button"
                      onClick={() => onNavigate("samuel-ai")}
                      className="shrink-0 rounded-xl border border-white/[.07] px-3 py-2 text-[10px] font-semibold text-white/45 transition hover:border-cyan-300/20 hover:text-cyan-100"
                    >
                      Pedir ao Samuel
                    </button>
                  </div>
