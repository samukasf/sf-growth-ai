    ...
    <section className="min-h-[calc(100dvh-130px)] overflow-hidden rounded-[28px] border border-white/[.07] bg-[#06101a] text-white shadow-[0_24px_80px_rgba(0,0,0,.28)]">
      <header className="flex flex-col gap-4 border-b border-white/[.07] bg-[linear-gradient(180deg,rgba(8,31,53,.86),rgba(5,17,29,.72))] p-4 sm:p-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[.2em] text-cyan-200/55">Samuel Studio</p>
            <h2 className="mt-1 text-xl font-semibold">Sites & Apps</h2>
            <p className="mt-1 text-xs text-white/40">Crie, salve, reabra, arquive e exporte projetos sem perder o trabalho.</p>
          </div>
        </div>
        <div className="flex flex-wrap gap-2">
          <button type="button" onClick={newProject} className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/[.04] px-3.5 py-2.5 text-xs font-semibold text-white/75 hover:bg-white/[.08]"><Plus className="size-4" /> Novo</button>
          <button type="button" onClick={saveCurrent} className="flex items-center gap-2 rounded-xl border border-cyan-300/20 bg-cyan-300/[.07] px-3.5 py-2.5 text-xs font-semibold text-cyan-50 hover:bg-cyan-300/[.12]"><Save className="size-4" /> Salvar</button>
          <button type="button" onClick={archiveCurrent} className="flex items-center gap-2 rounded-xl border border-white/10 px-3.5 py-2.5 text-xs font-semibold text-white/55 hover:bg-white/[.05]"><Archive className="size-4" /> Arquivar</button>
        </div>
      </header>
      ...
      <div className="grid min-h-0 xl:grid-cols-[220px_390px_minmax(0,1fr)]">
        <aside className="border-b border-white/[.06] bg-black/10 p-3 xl:border-b-0 xl:border-r">
          <div className="flex gap-2">
            <button type="button" onClick={() => setShowArchived(false)} className={cn("flex-1 rounded-xl border px-3 py-2.5 text-[10px] font-semibold", !showArchived ? "border-cyan-300/20 bg-cyan-300/[.07] text-cyan-50" : "border-white/[.06] text-white/35")}>Ativos {activeCount}</button>
            <button type="button" onClick={() => setShowArchived(true)} className={cn("flex-1 rounded-xl border px-3 py-2.5 text-[10px] font-semibold", showArchived ? "border-cyan-300/20 bg-cyan-300/[.07] text-cyan-50" : "border-white/[.06] text-white/35")}>Arquivo {archivedCount}</button>
          </div>
          <div className="mt-3 max-h-[260px] space-y-1.5 overflow-y-auto xl:max-h-[calc(100dvh-285px)]">
            {!hydrated ? (
              <div className="h-16 animate-pulse rounded-xl bg-white/[.025]" />
            ) : visibleProjects.length === 0 ? (
              <div className="rounded-xl border border-dashed border-white/[.08] p-4 text-center text-[10px] text-white/28">Nenhum projeto aqui.</div>
            ) : visibleProjects.map((project) => (
              <button key={project.id} type="button" onClick={() => openProject(project)} className={cn("w-full rounded-xl border p-3 text-left transition", currentId === project.id ? "border-cyan-300/20 bg-cyan-300/[.06]" : "border-white/[.05] bg-white/[.018] hover:bg-white/[.035]")}>
                <div className="flex items-center gap-2"><FolderOpen className="size-4 shrink-0 text-cyan-200/45" /><strong className="truncate text-xs text-white/70">{project.name}</strong></div>
                <p className="mt-1 truncate text-[9px] text-white/24">{project.draft.mode === "app" ? "Mini-app" : "Site"} · {project.draft.segment}</p>
              </button>
            ))}
          </div>
        ...
        <section className="border-b border-white/[.06] p-4 xl:border-b-0 xl:border-r xl:p-5">
          ...
          <div className="mt-4 grid grid-cols-2 gap-2">
            {MODE_OPTIONS.map((option) => (
              <button key={option.value} type="button" onClick={() => { updateDraft("mode", option.value); setPreviewPage(option.value === "app" ? "app" : "home"); }} className={cn("rounded-2xl border p-3 text-left transition", draft.mode === option.value ? "border-cyan-300/25 bg-cyan-300/[.07]" : "border-white/[.07] bg-white/[.02]")}>
                <span className="flex items-center gap-2 text-xs font-semibold">{option.value === "app" ? <Smartphone className="size-4" /> : <Globe2 className="size-4" />}{option.label}</span>
                <span className="mt-1 block text-[10px] leading-relaxed text-white/30">{option.description}</span>
              </button>
            ))}
          </div>

          <div className="mt-4 grid gap-3">
            <Field label="Empresa" value={draft.businessName} onChange={(value) => updateDraft("businessName", value)} />
            <Field label="Segmento" value={draft.segment} onChange={(value) => updateDraft("segment", value)} />
            <Field label="Oferta principal" value={draft.offer} onChange={(value) => updateDraft("offer", value)} />
            <Field label="Objetivo comercial" value={draft.goal} onChange={(value) => updateDraft("goal", value)} />
            <div className="grid grid-cols-2 gap-2"><Field label="Local" value={draft.location} onChange={(value) => updateDraft("location", value)} /><Field label="Botão" value={draft.cta} onChange={(value) => updateDraft("cta", value)} /></div>
            <div className="grid grid-cols-2 gap-2"><Field label="WhatsApp" value={draft.whatsapp} placeholder="+351..." onChange={(value) => updateDraft("whatsapp", value)} /><Field label="Google Maps" value={draft.mapsQuery} onChange={(value) => updateDraft("mapsQuery", value)} /></div>
          </div>
          ...
          <div className="mt-5 grid grid-cols-2 gap-2">
            <button type="button" onClick={() => openHtmlPreview(html)} className="flex min-h-11 items-center justify-center gap-2 rounded-xl border border-white/[.08] bg-white/[.025] text-xs font-semibold text-white/60 hover:bg-white/[.05]"><ExternalLink className="size-4" /> Abrir preview</button>
            <button type="button" onClick={() => downloadHtml(html, filename)} className="flex min-h-11 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-500 text-xs font-semibold text-white"><Download className="size-4" /> Exportar HTML</button>
          </div>

          <div className="mt-5 grid gap-2 text-[10px] text-white/35">
            {[
              { icon: LayoutTemplate, label: "Preview", value: draft.mode === "app" ? "Site + mini-app" : "Site responsivo" },
              { icon: FileCode2, label: "Arquivo", value: filename },
              { icon: MessageCircle, label: "WhatsApp", value: draft.whatsapp ? "Configurado no projeto" : "Pendente" },
              { icon: MapPinned, label: "Maps", value: draft.mapsQuery ? "Configurado" : "Pendente" },
              { icon: ShieldCheck, label: "Persistência", value: "Salvo localmente no navegador" },
            ].map((item) => <div key={item.label} className="flex items-center gap-3 rounded-xl border border-white/[.05] bg-white/[.018] p-3"><item.icon className="size-4 shrink-0 text-cyan-200/45" /><div className="min-w-0"><strong className="block text-white/55">{item.label}</strong><span className="block truncate">{item.value}</span></div></div>)}
          </div>
        </section>

        <section className="flex min-h-[560px] flex-col bg-[#03080e] p-3 sm:p-4">
          <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div><p className="text-[9px] font-bold uppercase tracking-[.18em] text-cyan-300/60">Preview navegável</p><h3 className="mt-1 text-sm font-semibold">{projectName || draft.businessName}</h3></div>
            <div className="flex max-w-full overflow-x-auto rounded-full border border-white/10 bg-white/[.04] p-1">{previewPages.map((page) => <button key={page.id} type="button" onClick={() => setPreviewPage(page.id)} className={cn("shrink-0 rounded-full px-3 py-1.5 text-[9px] font-semibold", previewPage === page.id ? "bg-white text-slate-950" : "text-white/45")}>{page.label}</button>)}</div>
          </div>
