...
export function ExecutiveHome({
  ...
  const activeCompanyName = primaryCompany?.name ?? "Seu portfólio";
                ...
                >
                  ...
                  <div className="mt-4 space-y-3">
                    <p className="ds-body-sm leading-relaxed text-[var(--ds-text)]">
                      {hasCompanies ? `A empresa ${activeCompanyName} está pronta.` : "Olá, Samuel."}
                    </p>
                    <p className="ds-body-sm leading-relaxed text-[var(--ds-text-muted)]">
                      Sou seu Executivo Digital.
                    </p>
                    <p className="ds-body-sm leading-relaxed text-[var(--ds-text-muted)]">
                      Vou ajudá-lo a administrar sua empresa com contexto e dados protegidos.
                    </p>
                    <p className="ds-body-sm leading-relaxed text-[var(--ds-text-muted)]">
                      {hasCompanies
                        ? "Continue no painel do Samuel para analisar prioridades e executar ações."
                        : "Nosso primeiro passo será cadastrar sua empresa e iniciar o Company Brain."}
                    </p>
                  </div>
                  <div className="mt-6">
                    {hasCompanies ? (
                      <Link
                        href={samuelHref}
                        className="inline-flex h-11 items-center justify-center rounded-[var(--ds-radius-md)] bg-[var(--ds-primary)] px-5 text-sm font-medium text-[var(--ds-text-inverse)] transition hover:bg-[var(--ds-primary-hover)]"
                      >
                        Abrir Samuel AI
                      </Link>
                    ) : (
                      <DsButton size="lg" onClick={openModal}>Criar empresa</DsButton>
                    )}
                  </div>
                </DsCard>
                ...
                <DsCard padding="lg" className="flex flex-col gap-4">
                  <div className="space-y-2">
                    ...
                    <h2 className="ds-heading text-[var(--ds-text)]">
                      {hasCompanies ? "Revise as prioridades executivas." : "Cadastre sua empresa."}
                    </h2>
                    <p className="ds-body-sm leading-relaxed text-[var(--ds-text-muted)]">
                      {hasCompanies
                        ? "O Samuel consolida os dados disponíveis e mostra o próximo melhor passo."
                        : "Depois iniciaremos automaticamente o Company Brain."}
                    </p>
                  </div>
                  {hasCompanies ? (
                    <Link href={samuelHref} className="text-sm font-semibold text-[var(--ds-primary)] hover:underline">
                      Ir para o painel executivo →
                    </Link>
                  ) : (
                    <DsButton onClick={openModal}>Criar empresa</DsButton>
                  )}
                </DsCard>
              </div>
