...
function computeFreeSlots(
  ...
  const configuredDayEnd = new Date(timeMin);
  configuredDayEnd.setHours(WORKDAY_END_HOUR, 0, 0, 0);
  const requestedEnd = new Date(timeMax);
  const dayEnd = Number.isNaN(requestedEnd.getTime())
    ? configuredDayEnd
    : new Date(Math.min(configuredDayEnd.getTime(), requestedEnd.getTime()));
